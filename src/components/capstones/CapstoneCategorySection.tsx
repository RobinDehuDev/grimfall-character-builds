...
import {
  capstoneIconUrl,
  capstoneIdSetToSlots,
  slotsToCapstoneIdSet,
} from "@/lib/capstones";
import type { CapstoneGameItem, GameItem } from "@/lib/types";
...
import { CapstonePickerModal } from "./CapstonePickerModal";

interface CapstoneCategorySectionProps {
  meta: CategoryMeta;
  slots: (string | null)[];
...
export function CapstoneCategorySection({
  meta,
  slots,
  activeSlot,
  onSlotClick,
  ...
  getItemById,
  className,
}: CapstoneCategorySectionProps) {
  const { t } = useTranslation();
  const { label } = useCategoryLabel(meta.key);
  const [expanded, setExpanded] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);

  const maxSlots = BUILD_SLOTS.capstone;
  const filled = slots.filter(Boolean).length;
  const isComplete = filled === maxSlots;
  const borderColor = itemBorderColor(meta.key);

  const selectedIds = useMemo(() => slotsToCapstoneIdSet(slots), [slots]);

  const handleSelectionChange = (ids: Set<string>) => {
    onSlotsChange(capstoneIdSetToSlots(ids));
  };
          ...
          {expanded && (
            <div className="flex flex-col gap-2">
              {slots.map((slotId, index) => {
                ...
                const capstone =
                  item?.type === "capstone" ? (item as CapstoneGameItem) : undefined;
                ...
                return (
                  ...
                  >
                    ...
                    {capstone ? (
                      <ItemNameTooltip
                        item={capstone}
                        className="slot__name slot__name--filled flex items-center gap-2 text-quality-legendary"
                      >
                        {capstone.icon && (
                          <img
                      ...
                      <ItemNameTooltip
                        item={item}
                        className="slot__name slot__name--filled text-quality-legendary"
                      >
                        {item.name}
                      </ItemNameTooltip>
                    ) : (
